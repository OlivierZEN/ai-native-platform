...
使用范围：对象作为字段、关系、记录、对象授权策略、数据范围和共享规则的共同边界。对象只能在草稿版本修改或删除；草稿删除会级联删除同版本字段，但存在关系引用时拒绝。已发布对象只有在没有任何记录且没有关系依赖时，才能通过候选版本和 Changeset 从当前快照移除；历史已发布版本保持不可变。
...
从未发布的草稿字段可以直接删除。已发布字段只有在所有记录中都没有该 API key 时，才能从候选版本直接移除；仍有存量值时必须沿以下生命周期完成弃用和清理，不能用直接删除绕过 purge。
