...
- PostgreSQL 16 全仓和 `go test -race ./... -count=1` 通过；定向主接线/授权测试、vet、module verify、状态与 diff 校验通过。合并 TASK-060 后目录应为 57 项能力；26 项人类默认 scope 加 1 项服务身份专用 scope 的 19 项认证回归待合并后复验。
