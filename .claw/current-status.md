...
updated_by: ai under explicit Keycloak production authorization
phase: cross-product-controlled-provisioning
active_task: "TASK-029"
next_action: "Deploy Keycloak at sso.agentcici.com with isolated runtime/database, then implement and release AgentCiCi/Semattice OACT integration."
read_next:
  goals: true
  decisions: true
  issue_list: false
...
- TASK-029 基础设施阶段完成：Keycloak 26.7.0 已运行于授权 ECS 的 loopback `:8180`，Nginx 经现有 `*.agentcici.com` TLS 证书公开 `https://sso.agentcici.com`；独立 PostgreSQL `keycloak` 数据库/role、非特权 systemd、业务 Realm `agentcici` 和四个首批官方 client 已创建。OIDC Discovery/JWKS 均为 HTTPS 200，公网 health/metrics 为 404，Semattice HTTPS 200 未受影响。应用 OACT/JWKS 接入仍在实施，不能把本轮基础设施验收表述为完整 SSO 已切流。
