...
next_action: "Monitor the first operator browser login and investigate any remaining tenant-detail UI-only error with its exact URL/request ID; the identity and tenant binding path is live."
read_next:
  goals: true
  decisions: true
  issue_list: false
