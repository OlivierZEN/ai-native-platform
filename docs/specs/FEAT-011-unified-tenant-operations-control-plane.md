...
task_ids: TASK-011
related_decisions: ADR-006, ADR-009
related_issues: none
...
本功能把既有运营管理端扩展为产品无关的统一租户运营控制面。开通任一首个产品前，运营端先创建或解析全局租户并分配 `tenant_id + org_id`；Agent CC 和 Native Platform 分别维护独立产品订阅。某个客户需要同时使用并绑定两个产品时，两项产品订阅必须归属于同一个全局租户并使用完全相同的标识，不能在产品之间再做第二层租户映射。
...
- 运营控制面统一生成随机 `UUIDv4 tenant_id` 和一对一的 20 位 `org_id`。
- 同一全局租户下的 Agent CC 与 Native Platform 使用同一个 `tenant_id + org_id`；`org_id` 作为稳定、可读的业务编号和兼容标识。
- Native Platform 接收针对全局租户的独立开通、暂停、恢复、套餐/配额变更和注销编排请求，并返回独立的产品侧状态；不要求 Agent CC 已开通。
- Native Platform 在本地 `tenant_registry` 中维护 `tenant_id`、`org_id`、分片、bucket、路由版本、配额投影和产品侧生命周期状态。
...
- 把 shard、region、bucket、套餐或产品类型编码进 `tenant_id` 或 `org_id`。
- 在本仓库实现既有运营管理端的前端页面。
...
3. 客户以后需要把另一个产品绑定到已有租户：运营端选择已有全局租户，第二个产品必须复用完全相同的 `tenant_id + org_id`，不得重新生成或替换。
...
6. Agent CC 已存在的租户在接入前先盘点 `org_id` 并回填 `tenant_id`；这不会自动开通 Native。
7. 支持、审计和计费系统以 `tenant_id` 关联机器数据，以 `org_id` 展示和检索企业租户，并分别查看 Agent CC 与 Native 产品状态。
...
| `org_id` | `varchar(20)` | 统一运营控制面 | Agent CC 兼容、运营展示、API/CLI/MCP 可读业务编号 |
...
`tenant_id` 与 `org_id` 必须一对一且终身稳定。`tenant_id` 由统一运营控制面使用密码学安全随机源生成 UUIDv4，不复用、不中途修改、不编码创建时间或物理位置；租户迁移只更新 Native Platform 的 `shard_id`、`tenant_bucket` 和 `route_revision`。
...
新租户的 `org_id` 延续已确认的 20 位约定。若采用 `org` 加 17 位小写字母数字格式，入口和数据库应使用等价于 `^org[a-z0-9]{17}$` 的约束；存量不合规 ID 必须在迁移清单中显式处理，不能静默截断或重新生成。

### 所有权边界

```text
Unified Tenant Operations Control Plane
  owns: tenant_id, org_id, global tenant lifecycle, per-product status
...
- 两个平台分别保存产品侧授权和审计，使用 `tenant_id`、`org_id`、`subject_id`、`operation_id`、`request_id` 和委托链关联。
...
- 请求 payload 或普通 header 中出现的 `tenant_id` 只能作为待核对值；受信 TenantContext 必须由已验证的运营/身份令牌、`tenant_id + org_id` 一对一目录和 Native active 投影共同解析。
...
sequenceDiagram
    ...
    alt 开通 Agent CC
        O->>P: provision agent_cc(统一 tenant_id + org_id)
        P-->>O: Agent CC product result
    else 开通 Native Platform
        O->>N: tenant.provision(统一 tenant_id + org_id)
        N->>N: 校验全局租户与 ID 一致
...
| `tenant.get_status` | 查询 Native 产品侧开通和生命周期状态 | 返回全局/产品修订号、路由版本和可重试错误 |
| `tenant.suspend` | 暂停 Native 产品访问 | 不影响另一个产品，不删除数据；高风险审批和审计 |
| `tenant.resume` | 恢复 Native 产品访问 | 必须验证全局状态和产品修订号 |
| `tenant.update_entitlement` | 更新套餐、配额和能力投影 | 版本化、可回放、可审计 |
...
1. 导出并核验 Agent CC 现有组织清单，确认 `org_id` 唯一性、格式、状态和重复/异常数据。
2. 在统一运营控制面为每个现有 `org_id` 分配唯一 UUIDv4 `tenant_id`，建立不可变一对一目录。
...
- [ ] 开通任一首个产品时只生成一组稳定的 `tenant_id + org_id`；未开通产品保持 `not_provisioned`。
- [ ] 后续绑定第二个产品时必须选择已有全局租户并复用完全相同的 `tenant_id + org_id`；每个全局租户对每种产品最多一个投影。
...
- [ ] 缺少运营身份、错误 audience、伪造 `tenant_id/org_id`、重复操作、乱序修订和跨租户访问均被拒绝。
...
- 存量 `org_id` 不满足 20 位约束：进入异常清单并制定显式兼容方案，不截断、不覆盖。
...
- 回滚只关闭当前产品编排入口并暂停批次；已经分配的 `tenant_id` 和 `org_id` 永不回收复用，另一个产品不受影响。

## 实现进展

- 当前状态：设计已由用户确认；用户于 2026-07-18 扩大 L2 授权，允许在本仓库实施 PostgreSQL、本地租户控制面和共享身份集成，但尚未开始本功能的接口、迁移或数据库变更。
- 已完成：统一标识、所有权边界、开户编排、迁移原则和验收标准。
- 未完成：既有运营接口及共享身份依赖盘点、存量 Agent CC 租户审计、产品无关全局租户/订阅契约落地、Native Capability 实现、PostgreSQL PoC 和端到端验证。
