...
- CLI、MCP stdio 与公开发现、认证调用的 Streamable HTTP MCP 配置示例。
...
- MCP 同时支持受信主机的 stdio 和 Streamable HTTP；公网 `/mcp` 只经 Nginx 443 反向代理到 loopback 应用，匿名发现、认证调用，并关闭代理 buffering。Nginx 到 loopback 的 `Host` 固定为 `127.0.0.1`，以保留 SDK DNS-rebinding 防护。
...
Internet
  ...
  -> Nginx
       ...
       /mcp              127.0.0.1:8080 public-discovery/authenticated-call MCP
...
- [x] 未授权 API 调用返回 401。
- [x] 公网匿名 `initialize`、`tools/list` 成功；匿名 `tools/call` 返回 401，确认 Nginx `/mcp` 精确代理及调用鉴权边界已生效。
